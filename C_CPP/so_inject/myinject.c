...
long ptrace_call(pid_t pid, uint32_t addr, long *params, uint32_t num_params, struct user_regs_struct * regs)
{
    ...
    // pid ： 目标进程  regs->esp ： 目标地址  params ： 参数  最后一个参数：要写入的长度
...
int inject_remote_process(pid_t target_pid, const char *library_path, const char *function_name, const char *param, size_t param_size)
{
    ...
    if (sohandle == NULL){
        if (ptrace_call_wrapper(target_pid, "dlerror", dlerror_addr, parameters, 0, &regs) == -1)
            goto exit;
        /*
        void * error_str = ptrace_retval(&regs);
        char * read_error;
...
int main(int argc, char** argv)
{
	...
	if (target_pid != -1)
	{
		inject_remote_process(target_pid, argv[2], "hook_entry",  argv[1], strlen(argv[1]));
	}
